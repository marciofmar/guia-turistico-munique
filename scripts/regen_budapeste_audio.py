...
"""
Gera áudios de narração para os POIs de Budapeste.
Usa Microsoft Edge TTS (gratuito, sem API key).

Voz principal: pt-BR-FranciscaNeural
Nomes húngaros são pronunciados pela voz hu-HU-NoemiNeural via segmentação.

Uso:
    # Gerar apenas os novos POIs adicionados:
    python scripts/regen_budapeste_audio.py

    # Gerar POI específico:
    python scripts/regen_budapeste_audio.py gellert-hegy

    # Gerar todos os POIs de Budapeste (incluindo os existentes):
    python scripts/regen_budapeste_audio.py --all

    # Forçar regeneração mesmo se o arquivo já existe:
    python scripts/regen_budapeste_audio.py --all --force
    python scripts/regen_budapeste_audio.py gellert-hegy --force
"""
...
# POIs novos adicionados — gerados por padrão (sem --all)
NEW_POIS = {
    "gellert-hegy",
    "sinagoga-dohany",
    "mercado-central",
...
# Termos húngaros para pronunciar com voz húngara
FOREIGN_LANG_TAGS = [
    ...
    ("Hősök tere", "hu-HU"),
    ("Széchenyi", "hu-HU"),
    ("Széchenyi Gyógyfürdő", "hu-HU"),
    ("Andrássy", "hu-HU"),
    ("Szabadság tér", "hu-HU"),
    ("Szabadság-szobor", "hu-HU"),
    ("Dohány utcai zsinagóga", "hu-HU"),
    ("Szimpla Kert", "hu-HU"),
    ("Kőleves", "hu-HU"),
    ("Margit-sziget", "hu-HU"),
    ("Margit", "hu-HU"),
    ("Földalatti Vasút", "hu-HU"),
    ("Nagy Vásárcsarnok", "hu-HU"),
    ("Gerbeaud", "hu-HU"),
    ("Borkonyha", "hu-HU"),
    ("Rivalda", "hu-HU"),
    ("pálinka", "hu-HU"),
...
async def main():
    ...
    json_path = os.path.join(ROUTES, "budapeste.json")
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)

    pois = data.get("pois", [])

    # Filtrar quais POIs processar
    if filter_ids:
        pois = [p for p in pois if p["id"] in filter_ids]
    elif not all_pois:
        pois = [p for p in pois if p["id"] in NEW_POIS]

    # Filtrar POIs sem campo audio definido
    tasks = []
    for poi in pois:
        audio_rel = poi.get("media", {}).get("audio", "")
        if not audio_rel:
            print(f"[SKIP] {poi['id']} — sem campo audio no JSON")
            continue
    ...
    print(f"Gerando {len(tasks)} narração(ões) de Budapeste...\n")

    for i, (poi_id, text, out) in enumerate(tasks, 1):
        print(f"[{i}/{len(tasks)}] budapeste/{poi_id}")
        try:
