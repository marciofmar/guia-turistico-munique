...
"""
Gera áudios de narração para os POIs de Nuremberg.
Voz: pt-BR-FranciscaNeural (consistente com Praga, Budapeste, Bratislava, Český Krumlov)

Uso:
    python scripts/regen_nuremberg_audio.py              # apenas novos POIs
    python scripts/regen_nuremberg_audio.py --all        # todos
    python scripts/regen_nuremberg_audio.py durer-haus   # POI específico
    python scripts/regen_nuremberg_audio.py --force      # força regeneração
"""
...
async def main():
    ...
    json_path = os.path.join(ROUTES, "nuremberg.json")
    ...
    print(f"Gerando {len(tasks)} narracao(oes) de Nuremberg...\n")

    for i, (poi_id, text, out) in enumerate(tasks, 1):
        print(f"[{i}/{len(tasks)}] nuremberg/{poi_id}")
        try:
